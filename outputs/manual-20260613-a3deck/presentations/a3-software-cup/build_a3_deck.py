...
def build_deck():
    ...
    add_kicker(slide, "软件杯 A3 赛题", width=Inches(1.75))
    ...
    slides_notes.append(
        "第 1 页讲稿：这套系统的核心，不是单独做好某一个 AI 功能，而是把学习画像、资源生成、问答辅导、路径规划和评估反馈真正连成一条闭环。评委可以先记住一句话，大模智学想解决的不是“能不能生成”，而是“能不能持续支持一个学生学下去”。"
    )
    ...
    layers = [
        ("表现层", "首页 / 智能问答 / 能力测试 / 学习报告 / 学习计划 / 赛题对照"),
    ...
    add_image_cover(slide, screenshot_dir / "competition_readiness_fixed.png", Inches(8.76), Inches(5.55), Inches(3.0), Inches(1.08))
    ...
    for title, status, color in rows:
        ...
        run.font.color.rgb = rgb(color)
        y += Inches(0.7)
    add_image_cover(slide, screenshot_dir / "competition_readiness_fixed.png", Inches(6.28), Inches(2.2), Inches(6.0), Inches(4.9))
    add_footer(slide, "来源：docs/A3_document_checklist.md / screenshots/competition_readiness_fixed.png")
    slides_notes.append(
    ...
    out_path = output_dir / "大模智学_A3答辩初稿_深墨蓝科技风.pptx"
    prs.save(out_path)

    notes_lines = ["# 大模智学 A3 答辩初稿讲稿", ""]
    for idx, note in enumerate(slides_notes, start=1):
        notes_lines.append(f"## 第 {idx} 页")
        notes_lines.append(note)
        notes_lines.append("")
    (output_dir / "大模智学_A3答辩初稿_讲稿备注.md").write_text("\n".join(notes_lines), encoding="utf-8")
