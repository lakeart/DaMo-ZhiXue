...
def api_learning_plan_data(student_id):
    try:
        ...
        period = _clamp_int(request.args.get('period'), 7, 7, 30)
        ...
        resources = recommend_learning_resources(student.username, topic_mastery_data, goal=goal)
...
def generate_learning_plan(student_username, topic_mastery_data, plan_period=7, goal='mastery', selected_topics=None):
    selected_topics = selected_topics or []
    topic_lookup = {item.get('topic'): item for item in topic_mastery_data if item.get('topic')}

    if selected_topics:
        ranked_topics = [topic_lookup.get(topic, {'topic': topic, 'mastery': 45}) for topic in selected_topics]
        ranked_topics.extend(
            item for item in sorted(topic_mastery_data, key=lambda x: x.get('mastery', 0))
            if item.get('topic') not in selected_topics
        )
    else:
        ranked_topics = sorted(topic_mastery_data, key=lambda x: x.get('mastery', 0))
    ...
    focus_count = min(6, max(4, min(plan_period, 14) // 2 + 1))
    focus_topics = ranked_topics[:focus_count]
    weekday_labels = ['周一', '周二', '周三', '周四', '周五', '周六', '周日']
    time_slots = ['上午', '下午', '晚上']
    goal_duration_bias = {'mastery': 0, 'exam': 10, 'project': 15}.get(goal, 0)
    ...
    for item in focus_topics:
        ...
        duration = min(95, 35 + max(0, 70 - mastery) + goal_duration_bias)
        topic_cards.append({
    ...
    weekly = []
    for index in range(plan_period):
        topic = topic_cards[index % len(topic_cards)]
        study_date = date.today() + timedelta(days=index)
        slot = time_slots[index % len(time_slots)]
        weekly.append({
            'date': study_date.isoformat(),
            'day': weekday_labels[study_date.weekday()],
            'time': slot,
            'topic': topic['topic'],
    ...
    return {'weekly': weekly, 'topics': topic_cards}

def recommend_learning_resources(student_username, topic_mastery_data, goal='mastery'):
    focus_topics = sorted(topic_mastery_data, key=lambda item: item.get('mastery', 0))[:4]
    if not focus_topics:
        focus_topics = [{'topic': '数据结构', 'mastery': 52}, {'topic': '算法分析', 'mastery': 46}]
    ...
    for item in focus_topics:
        ...
        videos.append({
            'title': f'{topic} 精讲微课',
            'match': min(98, 65 + max(0, 75 - mastery))
        })
        exercises.append({
            'title': f'{topic} {("冲刺训练" if goal == "exam" else "专项练习")}',
            'priority': '优先完成' if mastery < 60 else '建议完成',
            'class': 'bg-danger' if mastery < 45 else 'bg-warning' if mastery < 60 else 'bg-success'
        })

    return {'videos': videos, 'exercises': exercises, 'articles': []}
